...
$ pkispawn -s KRA -f kra_inst -vv            
                \end{lstlisting}
            \end{enumerate}
        \item Enroll a certificate for user Foo1 and archive Private Key in KRA instance
            \begin{enumerate}[label*=\arabic*.]
...
$  pki -d /opt/rhqa_pki -c 'Secret123'\
    ...
    <certificate_serial_Number_of Agent_Cert>
                            \end{lstlisting}
                    \end{enumerate}
                \item  Archive a passphrase(password) using Agent User
                    \begin{lstlisting}[style=bashInputStyle]
$ pki -d /opt/rhqa_pki -c 'Secret123' \
    -h pki1.example.org -p 14080 \
...
------------------------
  ...
  Status: complete
                    \end{lstlisting}
                \item Generate Symmetric keys with AES algo of size 128 and archive it
                    \begin{lstlisting}[style=bashInputStyle]
$ pki -d /opt/rhqa_pki -c 'Secret123' \
    -h pki1.example.org -p 14080 \
