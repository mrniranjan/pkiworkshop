...
\subsubsection{Exercises}
    \begin{enumerate}[label*=\arabic*.]
        \item Require 2 RHEL7/Fedora 23 systems , setup hostnames as specified below and make setup hostname
            resolutions through \textit{/etc/hosts}:
            \begin{lstlisting}
pki1.example.org
pki2.example.org
            \end{lstlisting}
        \item Configure vncserver on both systems
        \item Setup CS9 and RHEL7 Optional Repos
            \begin{lstlisting}[style=BashInputStyle]
...
gpgcheck=0
        ...
        \item Setup Install \textbf{\textit{redhat-pki}} and \textbf{\textit{389-ds-base}} package 
...
$ yum install 389-ds-base
        ...
        \item Setup CA Subsystem using pkispawn in interactive mode
            \begin{enumerate}[label*=\arabic*.]
...
$ setup-ds.pl --silent --file=ca-ds.inf --debug
                ...
                \item \label{ca1}Run \textbf{\textit{pkispawn}} in interactive mode use default ports and specify Directory
                    server information configured above. Use Tomcat Instance name as Example1-RootCA
            \end{enumerate}
...
$ pkidaemon status tomcat Example1-ExtCA1
                    \end{lstlisting}
                \end{enumerate}
            \item Configure Clone of RootCA configured in \ref{ca1}
                \begin{enumerate}[label*=\arabic*.]
                    \item Features of clone
                        ...
                        \end{itemize}
                    \item Create a Backup of RootCA's System Certs in pkcs12 format using PKCS12Export
                        \begin{lstlisting}[style=bashInputStyle]
$ PKCS12Export -d /var/lib/pki/Example-RootCA1/alias -p /tmp/nssdb-pass  -w /tmp/pkcs12-pass -o ca_backup_keys.p12
                        \end{lstlisting}
...
https://github.com/mrniranjan/pkiworkshop/blob/master/san.csr
        ...
        \item Use \textbf{pki} cli and try different options
            \begin{enumerate}[label*=\arabic*.]
                \item Create a NSS database using pki client cli with password 'Secret123'
                    \begin{lstlisting}[style=bashInputStyle]
$ pki -d /opt/rhqa_pki -c 'Secret123' client-init                                    
                    \end{lstlisting}
                \item Import CA Admin Cert of RootCA(\ref{ca1}) to /opt/rhqa\_pki
                    \begin{lstlisting}[style=bashInputStyle]
$ pki -d /opt/rhqa_pki -c 'Secret123' client-cert-import --pkcs12 /root/.dogtag/Example1-RootCA/ca_admin_cert.p12  --pkcs12-password Secret123                                    
                    \end{lstlisting}
                \item Import CA Cert of RootCA(\ref{ca1})
                    \begin{lstlisting}[style=bashInputStyle]
$ pki -d /opt/rhqa_pki1 -c 'Secret123' -h pki1.example.org -p 8080 client-cert-import "RootCA" --ca-server                                    
                    \end{lstlisting}
                \item Submit a certificate request for user "foo" with subject DN:
                    \begin{lstlisting}
...
$ pki -d /opt/rhqa_pki -c 'Secret123' \
    ...
    "UID=foo,E=foo@example.org,CN=Foo User1,O=Example Org" 
                    \end{lstlisting}
                \item Approve the Certificate request using Admin Cert
                    \begin{lstlisting}[style=bashInputStyle]
