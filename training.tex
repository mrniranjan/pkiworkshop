%
% Latex file containing contents of PKI and CS Workshop
% Copyright (C) 2015 Niranjan M.R
%
...
$ PKCS12Export -d /var/lib/pki/pki-tomcat/alias -p /tmp/nssdb-pass  -w /tmp/pkcs12-pass -o ca_backup_keys.p12
                        ...
                        \texttt{"Internal"} in the file \texttt{/var/lib/pki/pki-tomcat/conf/password.conf}. 
                            \item File \texttt{/tmp/pkcs12-pass} should be craated with the passwod that you want set on
                                the backup file. 
                        \end{itemize}
                    \item Copy \textit{ca\_backup\_keys.p12} to the Clone system
...
https://github.com/mrniranjan/pkiworkshop/blob/master/san.csr
                        ...
                        \item Enable the profile from Agent Interface
                            \begin{itemize}
                                ...
                                \item Select \textbf{``Manual Server Certificate with SAN from CSR''}
                                \item Click on \textbf{``Approve''}
                            \end{itemize}
                        \item From EE page Select profile \textbf{``Manual Server Certificate with SAN from CSR''} and submit the CSR
...
$ pki -d /opt/rhqa_pki -c Secret123 \
    ...
    --type crmf --transport /opt/rhqa_pki/transportCert.pem                    
                    \end{lstlisting}
